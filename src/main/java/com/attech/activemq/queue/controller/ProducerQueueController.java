package com.attech.activemq.queue.controller;
...
public class ProducerQueueController {
    ...
    public Map<String, Object> queueProduceMessage() throws Exception {
        ...
        // 3.开启连接（ActiveMQ创建的连接是需要手动开启的）  注意不是open。。。
        connection.start();
        // 4.获取session（类似于Hibernate中的session，都是用会话来进行操作） 里面有两个参数，参数1为是否开启事务，参数2为消息确认模式
